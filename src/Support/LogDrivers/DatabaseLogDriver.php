...
class DatabaseLogDriver implements LogDriverInterface
{
    public function write(array $payload, string $logFileName, string $level): void
    {
        ...
        if ($retryGroupId === '') {
            $retryGroupId = $eventHash ?? ($this->hashFromParts([$occurredAt, $label]) ?? 'unknown');
        }
        ...
        $row = [
            ...
            'context'         => $this->encodeJson($contextPayload),
    ...
    protected function loggingTable(): string
    {
    ...
    protected function hashFromParts(array $parts): ?string
    {
        $string = implode('|', array_map(fn ($part) => is_scalar($part) ? (string) $part : '', $parts));
